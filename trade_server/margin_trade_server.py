...
class TraderServer(AsyncBaseTradeServer):
    ...
    def getBuyAction(self, quantity):
        return "1"

    def getSellAction(self, ticker, quantity):
        holding = self.getAccountHolding(ticker)

        if holding is None:
            return "B"

        if holding.getLongAvailable() >= quantity:
            return "2"
    ...
    async def sendLimitOrder(self, order: LimitOrder):
        ...
        if order.isBuy():
            ...
            action = self.getSellAction(order.getTicker(), order.getQuantity())

        order.setCustId(self.genCustOrderId())
        req = ('O',order.getCustId(),'C',self.__accountid,'',order.getTicker(),action,order.getQuantity(),order.getLimitPrice(),'0')
    ...
    async def _cancelOrderReq(self, order_ids: list):
        ...
        for order_id in order_ids:
            req.append(('C','','C',self.__accountid,order_id))
    ...
    async def sendOrdersInBatch(self, batch_id, order_list: typing.List[LimitOrder]):
        ...
        for order in order_list:
            if order.isBuy():
                ...
                action = self.getSellAction(ticker=order.getTicker(), quantity=order.getQuantity())

            order.setCustId(self.genCustOrderId())
            req = ('O', order.getCustId(), 'C', self.__accountid, '', order.getTicker(), action, order.getQuantity(),
